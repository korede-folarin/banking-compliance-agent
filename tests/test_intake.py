...
def test_doc1_core_fields_match_source(doc1_fields):
    ...
    assert isinstance(f.key_clauses, list) and len(f.key_clauses) > 0
...
def test_doc2_has_no_vulnerable_customer_provision(doc2_fields):
    # loan_agreement_2.txt deliberately omits any vulnerable-customer
    # process. Extraction must not invent one.
    joined = " ".join(doc2_fields.key_clauses).lower()
    assert "vulnerab" not in joined
    assert "vulnerab" not in doc2_fields.fees.lower()


def test_doc1_and_doc3_have_vulnerable_customer_provisions(doc1_fields, doc3_fields):
    joined1 = " ".join(doc1_fields.key_clauses).lower()
    joined3 = " ".join(doc3_fields.key_clauses).lower()
    assert "vulnerab" in joined1
    assert "vulnerab" in joined3
